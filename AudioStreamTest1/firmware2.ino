...
const int wsMinSendInterval = 20;  // Send every 20ms
bool wsClientConnected = false;
bool audioStreamingActive = false;  // Flag to control audio streaming
    ...
    case WS_EVT_DATA:
      ...
      if (len) {
        ...
        if (command == "play") {
          ...
          digitalWrite(led_indicator, HIGH);  // Turn on LED indicator
          Serial.println("Audio streaming started");
        } else if (command == "stop") {
          audioStreamingActive = false;
          digitalWrite(led_indicator, LOW);  // Turn off LED indicator
...
void loop() {
  ...
  // Only process and send audio data if streaming is active
  if (audioStreamingActive && wsClientConnected) {
    ...
    if (result == ESP_OK && bytes_read > 0) {
      ...
      if (currentTime - lastWsSendTime >= wsMinSendInterval) {
    ...
    // If not streaming, add a small delay to prevent CPU hogging
    delay(100);
  }
  // Small delay in both cases
  delay(2);
}
