...
<html>
<head>
    <title>NoviBell Audio Streaming Test</title>
...
<body>
    ...
    <script>
        ...
        let dataPoints = [];
        const maxDataPoints = 50; // Increased for higher sample rate
        let isPlaying = false;
        let audioContext;
        let audioBuffer = [];
        const maxAudioBufferSize = 32768; 
        let gainNode;
        ...
        // Handle incoming WebSocket messages (now binary data)
        function handleWebSocketMessage(evt) {
            try {
                ...
                if (evt.data instanceof ArrayBuffer) {
                    ...
                    if (samples.length > 0) {
                        // Add samples to visualization buffer
                        // Take every Nth sample for visualization to prevent overwhelming
                        const skipRate = Math.max(1, Math.floor(samples.length / 50));
